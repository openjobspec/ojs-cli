...
func detectFrameworks(dir string) []frameworkDetect {
	results := detectSidekiq(dir)
	results = append(results, detectBullMQ(dir)...)
	results = append(results, detectCelery(dir)...)
	return results
}

func detectSidekiq(dir string) []frameworkDetect {
...
func detectBullMQ(dir string) []frameworkDetect {
...
func detectCelery(dir string) []frameworkDetect {
	var results []frameworkDetect
...
func migrateValidateConfig(args []string) error {
	configFile, err := firstArgument(args)
	if err != nil {
		return err
	}
...
func firstArgument(args []string) (string, error) {
	if len(args) == 0 {
