...
func (g *Generator) Generate() error {
	...
	case LangRuby:
		return g.generateRuby()
	case LangDotNet:
		return g.generateDotNet()
	default:
...
func (g *Generator) generateJava() error {
	for _, jt := range g.manifest.JobTypes {
		...
		if err != nil {
			return fmt.Errorf("parsing Java template: %w", err)
		}
		var buf bytes.Buffer
		if err := tmpl.Execute(&buf, jt); err != nil {
...
var javaTemplate = `// AUTO-GENERATED by ojs codegen
package ojsjobs;

public class {{ pascalCase .Type }}Jobs {
    public record Args(
{{- range .Args }}
        {{ javaType .Type }} {{ camelCase .Name }}{{ end }}
    ) {}
}
`
...
func (g *Generator) generateRust() error {
	...
	if err != nil {
		return fmt.Errorf("parsing Rust template: %w", err)
	}
	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, g.manifest); err != nil {
...
var rustTemplate = `// AUTO-GENERATED by ojs codegen
use serde::{Serialize, Deserialize};

{{ range .JobTypes }}
#[derive(Debug, Serialize, Deserialize)]
pub struct {{ pascalCase .Type }}Args {
{{- range .Args }}
...
func (g *Generator) generateRuby() error {
	tmpl, err := template.New("ruby").Funcs(template.FuncMap{
		"pascalCase": toPascalCase,
		"snakeCase":  toSnakeCase,
	}).Parse(rubyTemplate)
	if err != nil {
		return fmt.Errorf("parsing Ruby template: %w", err)
	}
	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, g.manifest); err != nil {
...
# AUTO-GENERATED by ojs codegen

module OJSJobs
{{ range .JobTypes }}
  module {{ pascalCase .Type }}
    def self.enqueue(client, **args)
      client.enqueue("{{ .Type }}", args.values, queue: "{{ .Queue }}")
    end
  end
{{ end }}
end
...
func (g *Generator) generateDotNet() error {
	tmpl, err := template.New("dotnet").Funcs(template.FuncMap{
		"pascalCase": toPascalCase,
		"csharpType": toCSharpType,
	}).Parse(dotNetTemplate)
	if err != nil {
		return fmt.Errorf("parsing .NET template: %w", err)
	}
	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, g.manifest); err != nil {
...
var dotNetTemplate = `// AUTO-GENERATED by ojs codegen
using OpenJobSpec;
namespace OjsJobs;

{{ range .JobTypes }}
public record {{ pascalCase .Type }}Args(
{{- range .Args }}
